...
def _default_youtube_group(cursor):
    cursor.execute("SELECT * FROM youtube_video_groups WHERE is_default = 1 LIMIT 1")
    row = cursor.fetchone()
    if not row:
        raise RuntimeError("默认视频分组不存在")
    return row


def _resolve_youtube_group(cursor, group_id=None):
    if group_id in (None, ""):
        return _default_youtube_group(cursor)
    try:
        normalized_id = int(group_id)
    except (TypeError, ValueError) as exc:
        raise ValueError("groupId 必须是整数") from exc
    cursor.execute("SELECT * FROM youtube_video_groups WHERE id = ?", (normalized_id,))
    row = cursor.fetchone()
    if not row:
...
def create_youtube_video_group(name):
...
def rename_youtube_video_group(group_id, name):
    normalized_name = _normalize_youtube_group_name(name)
    init_youtube_video_table()
    try:
        with _db_connect(row_factory=True) as conn:
            cursor = conn.cursor()
            cursor.execute("BEGIN IMMEDIATE")
            group = _resolve_youtube_group(cursor, group_id)
            if bool(group["is_default"]):
                raise ValueError("默认分组不能重命名")
            cursor.execute(
                "UPDATE youtube_video_groups SET name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (normalized_name, int(group_id)),
            )
            cursor.execute('''
            SELECT groups.*, COUNT(videos.id) AS video_count
            FROM youtube_video_groups groups
...
def delete_youtube_video_group(group_id):
    init_youtube_video_table()
    with _db_connect(row_factory=True) as conn:
        cursor = conn.cursor()
        cursor.execute("BEGIN IMMEDIATE")
        ...
        cursor.execute(
            "SELECT COUNT(*) AS count FROM youtube_search_jobs WHERE group_id = ? AND status IN ('queued', 'running')",
            (int(group_id),),
        )
        if int(cursor.fetchone()["count"] or 0):
            raise RuntimeError("该分组存在运行中的检索任务，请等待任务结束后再删除")
        default_group = _default_youtube_group(cursor)
        cursor.execute("UPDATE youtube_videos SET group_id = ?, updated_at = CURRENT_TIMESTAMP WHERE group_id = ?", (default_group["id"], int(group_id)))
        moved_count = cursor.rowcount
        cursor.execute("UPDATE youtube_search_jobs SET group_id = NULL WHERE group_id = ?", (int(group_id),))
        cursor.execute("DELETE FROM youtube_video_groups WHERE id = ?", (int(group_id),))
        backend_logger.info(
...
def move_youtube_videos_to_group(video_ids, group_id):
    ...
    with _db_connect(row_factory=True) as conn:
        ...
        cursor.execute(f"SELECT video_id, group_id FROM youtube_videos WHERE video_id IN ({placeholders})", normalized_ids)
        ...
        cursor.execute(
            f"UPDATE youtube_videos SET group_id = ?, updated_at = CURRENT_TIMESTAMP WHERE video_id IN ({placeholders}) AND COALESCE(group_id, 0) != ?",
            [int(group["id"]), *normalized_ids, int(group["id"])],
        )
        moved_count = cursor.rowcount
        backend_logger.info(
...
def youtube_video_research_context(video_id):
    if not video_id:
        return {}
    init_youtube_video_table()
    with _db_connect(row_factory=True) as conn:
