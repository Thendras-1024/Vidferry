...
_WORKFLOW_ERROR_CONFIG = (
    ...
    (lambda _exc, text: "sign in to confirm you're not a bot" in text.lower() or "sign in to confirm you’re not a bot" in text.lower() or "use --cookies" in text.lower(), "VF-YOUTUBE-AUTH-REQUIRED", "YOUTUBE_AUTH_REQUIRED", "YouTube 要求登录验证，当前未配置有效的 YouTube Cookie；请在本机配置导出的 Cookie 文件后重试下载。"),
    (lambda _exc, text: "YOUTUBE_COOKIE_FILE_MISSING" in text, "VF-YOUTUBE-COOKIE-MISSING", "YOUTUBE_COOKIE_MISSING", "配置的 YouTube Cookie 文件不存在；请检查 YOUTUBE_COOKIE_FILE 后重试。"),
