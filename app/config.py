...
YOUTUBE_COOKIE_FILE = _env_text("YOUTUBE_COOKIE_FILE")
YOUTUBE_COOKIES_FROM_BROWSER = _env_text("YOUTUBE_COOKIES_FROM_BROWSER")
YOUTUBE_COOKIES_BROWSER_PROFILE = _env_text("YOUTUBE_COOKIES_BROWSER_PROFILE")
...
# 封面标题布局策略仅在代码中切换：vision 使用图片模型，rules 使用 OpenCV 规则布局。
COVER_LAYOUT_STRATEGY = "vision"
...
VIDEO_LOCAL_CLEANUP_MODE = _env_text("VIDEO_LOCAL_CLEANUP_MODE", "delete").lower()
if VIDEO_LOCAL_CLEANUP_MODE not in {"off", "report", "delete"}:
    VIDEO_LOCAL_CLEANUP_MODE = "delete"
...
# 先确定上下文窗口，再限制输出预算，避免错误配置产生负数输入预算。
AGENT_CONTEXT_MODEL_WINDOW_TOKENS = _env_int("AGENT_CONTEXT_MODEL_WINDOW_TOKENS", 1_000_000, 4096, 1_000_000)
AGENT_OUTPUT_MAX_TOKENS = _env_int(
    "AGENT_OUTPUT_MAX_TOKENS",
    65536,
    128,
    max(128, AGENT_CONTEXT_MODEL_WINDOW_TOKENS - 128),
)
# 旧名称只保留为运行时兼容别名，不再读取旧的 5K / 8K 上限。
AGENT_CHAT_MAX_TOKENS = AGENT_OUTPUT_MAX_TOKENS
AGENT_CONTEXT_RECENT_TURNS = _env_int(
    "AGENT_CONTEXT_RECENT_TURNS",
    max(2, _env_int("AGENT_CONTEXT_RECENT_MESSAGES", 8, 2, 20) // 2),
    2,
    10,
)
# 保留旧环境变量的换算语义，避免已有部署的热区窗口意外改变。
AGENT_CONTEXT_RECENT_MESSAGES = AGENT_CONTEXT_RECENT_TURNS * 2
...
AGENT_CONTEXT_OUTPUT_MAX_TOKENS = AGENT_OUTPUT_MAX_TOKENS
AGENT_CONTEXT_INPUT_MAX_TOKENS = AGENT_CONTEXT_MODEL_WINDOW_TOKENS - AGENT_CONTEXT_OUTPUT_MAX_TOKENS
AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS = _env_int(
    "AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS", min(850_000, AGENT_CONTEXT_INPUT_MAX_TOKENS),
    min(4096, AGENT_CONTEXT_INPUT_MAX_TOKENS), AGENT_CONTEXT_INPUT_MAX_TOKENS,
)
AGENT_CONTEXT_COMPACTION_RECOVERY_TOKENS = _env_int(
    "AGENT_CONTEXT_COMPACTION_RECOVERY_TOKENS", min(650_000, AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS),
    min(4096, AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS), AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS,
)
AGENT_CONTEXT_TOOL_RESULT_MAX_CHARS = _env_int("AGENT_CONTEXT_TOOL_RESULT_MAX_CHARS", 16_000, 500, 100_000)
AGENT_CONTEXT_TOOL_TOTAL_MAX_CHARS = _env_int("AGENT_CONTEXT_TOOL_TOTAL_MAX_CHARS", 64_000, 1_000, 300_000)
# 旧配置名保留为只读兼容别名；预算逻辑使用上面的绝对 token 字段。
AGENT_CONTEXT_SUMMARY_MAX_CHARS = 2_600_000
AGENT_CONTEXT_MAX_INPUT_TOKENS = AGENT_CONTEXT_INPUT_MAX_TOKENS
AGENT_CONTEXT_OUTPUT_RESERVE_TOKENS = AGENT_CONTEXT_OUTPUT_MAX_TOKENS
AGENT_CONTEXT_COMPACTION_TRIGGER_RATIO = AGENT_CONTEXT_COMPACTION_TRIGGER_TOKENS / AGENT_CONTEXT_INPUT_MAX_TOKENS
AGENT_CONTEXT_COMPACTION_RECOVERY_RATIO = AGENT_CONTEXT_COMPACTION_RECOVERY_TOKENS / AGENT_CONTEXT_INPUT_MAX_TOKENS
...
AUTH_PHONE_AUTO_REGISTER_ENABLED = _env_bool("VIDFERRY_AUTH_PHONE_AUTO_REGISTER_ENABLED", False)
...
CANDIDATE_ANALYSIS_MAX_JOBS = _env_int("CANDIDATE_ANALYSIS_MAX_JOBS", 1, 1, 2)
CANDIDATE_ANALYSIS_MAX_QUEUED_JOBS = _env_int("CANDIDATE_ANALYSIS_MAX_QUEUED_JOBS", 2, 0, 12)
CANDIDATE_ANALYSIS_MAX_ITEMS = _env_int("CANDIDATE_ANALYSIS_MAX_ITEMS", 3, 1, 5)
CANDIDATE_ANALYSIS_MAX_DURATION_SECONDS = _env_int("CANDIDATE_ANALYSIS_MAX_DURATION_SECONDS", 1800, 60, 14400)
YTDLP_DOWNLOAD_RETRIES = _env_int("YTDLP_DOWNLOAD_RETRIES", 10, 0, 30)
YTDLP_FRAGMENT_RETRIES = _env_int("YTDLP_FRAGMENT_RETRIES", 10, 0, 30)
YTDLP_DOWNLOAD_ATTEMPTS = _env_int("YTDLP_DOWNLOAD_ATTEMPTS", 3, 1, 5)
YTDLP_SOCKET_TIMEOUT_SECONDS = _env_int("YTDLP_SOCKET_TIMEOUT_SECONDS", 30, 5, 120)
