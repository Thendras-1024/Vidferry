...
def _remove_generated_default_group(cursor):
    ...
    if count != 1:
        return False
    cursor.execute("DELETE FROM youtube_video_groups WHERE name = %s AND is_default = 1", (DEFAULT_VIDEO_GROUP_NAME,))
    return True
...
def migrate_sqlite_to_postgres(sqlite_path, database_url):
    ...
    with sqlite3.connect(source) as sqlite_conn, psycopg.connect(database_url) as pg_conn:
        with pg_conn.cursor() as cursor:
            ...
            source_tables = {name for (name,) in sqlite_conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")}
            ...
            for table in ordered_tables:
                ...
                if rows:
                    ...
                    positions = [source_columns.index(column) for column in columns]
                    cursor.executemany(
                        f'INSERT INTO "{table}" ({quoted_columns}) VALUES ({placeholders})',
                        [[row[index] for index in positions] for row in rows],
                    )
                ...
                if rows and id_column and "INT" in str(id_column[2]).upper() and "id" in columns:
                    cursor.execute(f"SELECT setval(pg_get_serial_sequence('public.{table}', 'id'), COALESCE((SELECT MAX(id) FROM \"{table}\"), 1), true)")
            if "youtube_video_groups" in source_tables:
