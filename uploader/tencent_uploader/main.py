...
def _resolve_account_file(account_file: str | Path) -> str:
    ...
    if len(path.parts) == 1:
        return str((Path(BASE_DIR) / "cookiesFile" / "legacy" / "tencent_uploader" / path).resolve())
...
async def _wait_for_tencent_login(
    ...
    try:
        if page.url != original_url:
            url_changed_event.set()
        deadline = time.monotonic() + max(1, max_checks * poll_interval)
        while time.monotonic() < deadline:
        ...
        page.remove_listener("framenavigated", on_url_change)
