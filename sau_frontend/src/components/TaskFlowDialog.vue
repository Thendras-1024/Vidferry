  ...
  <el-dialog v-model="dialogVisible" class="task-flow-dialog" width="min(1240px, calc(100vw - 28px))" append-to-body destroy-on-close>
    <template #header>
      <div class="flow-dialog-heading">
        <div><strong>{{ detail?.task?.chineseTitle || '原题中文翻译未生成' }}</strong><small class="flow-dialog-english">{{ detail?.task?.englishTitle || detail?.task?.title }}</small><span>{{ detail?.task?.typeLabel }} · {{ detail?.task?.statusLabel }}</span></div>
    ...
    <div v-else-if="detail?.graph" class="task-flow-scroll">
      <div class="task-flow-canvas" :style="canvasStyle">
        ...
        <el-tooltip v-for="node in detail.graph.nodes" :key="node.id" placement="top" :disabled="!node.dependencySummary" effect="light">
          <template #content><div class="dependency-tooltip"><strong>依赖状态</strong><span v-if="node.dependencySummary?.completed.length">已完成：{{ node.dependencySummary.completed.join('、') }}</span><span v-if="node.dependencySummary?.reused.length">已复用：{{ node.dependencySummary.reused.join('、') }}</span><span v-if="node.dependencySummary?.running.length">进行中：{{ node.dependencySummary.running.join('、') }}</span><span v-if="node.dependencySummary?.pending.length">未开始：{{ node.dependencySummary.pending.join('、') }}</span><span v-if="node.dependencySummary?.blocked.length">异常阻塞：{{ node.dependencySummary.blocked.join('、') }}</span></div></template>
...
const maxColumn = computed(() => Math.max(1, ...(props.detail?.graph?.nodes || []).map(node => node.column || 0)))
...
const nodeStyle = node => ({ left: `${100 + (node.column || 0) * 102}px`, top: `${14 + (laneIndexes.value[node.laneId] || 0) * 86}px` })
const nodeById = id => (props.detail?.graph?.nodes || []).find(node => node.id === id)
const edgePath = edge => {
  const from = nodeById(edge.from)
  const to = nodeById(edge.to)
  if (!from || !to) return ''
  const fromX = 100 + (from.column || 0) * 102 + 54
  const toX = 100 + (to.column || 0) * 102 + 34
...
const tagType = status => ({ success: 'success', failed: 'danger', abnormal: 'danger', warning: 'warning', running: '', waiting: 'warning' }[status] || 'info')
const duration = seconds => seconds ? `${Math.round(Number(seconds))} 秒` : '未完成'
</script>

<style scoped lang="scss">
@use '@/styles/variables.scss' as *;
.flow-dialog-heading { display: flex; justify-content: space-between; align-items: center; padding-right: 20px; }
.flow-dialog-heading strong { display: block; color: $text-primary; font-size: 16px; }
.flow-dialog-english { display: block; overflow: hidden; max-width: 680px; margin-top: 3px; color: $text-secondary !important; font-size: 11px !important; text-overflow: ellipsis; white-space: nowrap; }
.flow-dialog-heading span { display: block; margin-top: 3px; color: $text-secondary; font-size: 12px; }
.flow-dialog-progress { color: $primary-color !important; font-size: 20px !important; font-variant-numeric: tabular-nums; }
...
.flow-node { position: absolute; z-index: 1; display: flex; width: 88px; height: 58px; flex-direction: column; align-items: center; padding: 0; border: 0; background: transparent; color: $text-primary; cursor: pointer; text-align: center; }
...
.flow-node strong { overflow: hidden; width: 88px; margin-top: 7px; font-size: 10px; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; }.flow-node small { margin-top: 2px; color: $text-secondary; font-size: 9px; }.flow-node-status { display: block; width: 22px; height: 22px; flex: 0 0 22px; border: 3px solid var(--vf-surface); border-radius: 50%; background: #c0c4cc; box-shadow: 0 1px 4px rgba(15, 35, 60, 0.16); }.node-success .flow-node-status { background: $success-color; }.node-running .flow-node-status { background: $primary-color; }.node-warning .flow-node-status { background: $warning-color; }.node-failed .flow-node-status, .node-abnormal .flow-node-status { background: $danger-color; }.node-waiting .flow-node-status { background: $warning-color; }.node-reused .flow-node-status { background: #9aa5b1; }.node-reused strong, .node-reused small { color: #7d8792; }
...
.dependency-tooltip { display: grid; gap: 4px; max-width: 260px; font-size: 11px; }.dependency-tooltip strong { font-size: 12px; }
@media (max-width: 680px) { .flow-dialog-heading { padding-right: 4px; }.flow-node-detail-grid { grid-template-columns: 64px 1fr 64px 1fr; }.flow-node-detail-grid span:nth-of-type(3) { grid-column: 1; }.flow-node-detail-grid b:nth-of-type(3) { grid-column: 2; } }
